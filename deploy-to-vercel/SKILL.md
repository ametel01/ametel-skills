...
**Do NOT** use `vercel project inspect`, `vercel ls`, or `vercel link` to detect state in an unlinked directory — without a `.vercel/` config, those commands can request terminal choices (or with `--yes`, silently link as a side-effect). Only `vercel whoami` is safe to run anywhere.
...
3. **If a git remote exists**, use repo-based linking with the selected team slug:
   ...
   **If there is no git remote**, fall back to standard linking:
   ...
   ```
   This may require terminal choices to select or create a project. It creates `.vercel/project.json`.
