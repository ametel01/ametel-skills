---
title: Monitoring
description: Essential PostgreSQL monitoring views, EXPLAIN plan reading, pg_stat_statements, logging, host metrics, and statistics management
tags: postgres, monitoring, explain, pg_stat_statements, logging, pgbadger, metrics, operations
---
...
For slow SELECTs, start with:

```sql
EXPLAIN (ANALYZE, BUFFERS, VERBOSE) <query>;
```

`ANALYZE` executes the statement. For UPDATE/DELETE/INSERT, run inside an explicit transaction and roll back, or omit `ANALYZE` when execution side effects are unsafe.

Look for:

- Large sequential scans on selective predicates, especially with high `Rows Removed by Filter`.
- Actual row counts far from estimates; run `ANALYZE` or inspect stale statistics before changing indexes.
- Nested loops with high `loops` counts over large inputs; missing join indexes or a bad join order are common.
- `shared read` much higher than `shared hit`; the query is doing physical I/O instead of cache reads.
- External sorts, hash batches, or temp blocks; check `work_mem`, result size, and sort/hash inputs.
- Index scans with many heap fetches; a covering index may help only when the hot query returns few columns.
