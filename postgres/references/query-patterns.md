...
tags: postgres, sql, query-optimization, n-plus-one, pagination, batch-inserts, upsert
---
...
**Batch small writes** — avoid one network round trip per row:
```sql
-- Bad: many individual INSERT statements from a loop
-- Good: one statement for a modest batch
INSERT INTO event (account_id, kind, payload)
VALUES
  (1, 'login', '{"ip":"10.0.0.1"}'),
  (1, 'logout', '{"duration":42}');
```

For very large loads, prefer `COPY` or driver bulk-copy APIs. Keep batches bounded so locks, WAL bursts, parameter counts, and rollback cost stay manageable.

**Use UPSERT for insert-or-update races** — do not check, then insert in separate statements:
```sql
INSERT INTO account_email (account_id, email, verified_at)
VALUES ($1, $2, now())
...
Use `RETURNING` to avoid a follow-up SELECT when the application needs generated IDs or changed values.
