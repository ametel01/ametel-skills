...
Named prepared statements, server-side statement caches, and driver auto-prepare features can break in transaction pooling because each transaction may use a different backend connection.

Use one of these patterns:

- Prefer unnamed prepared statements or parameterized simple queries for pooled OLTP traffic.
- Disable driver auto-prepare or server-side statement caching when the driver supports it.
- Use a direct connection for session-dependent workflows that require named prepared statements.
- Verify with the exact application driver; many errors appear only after a statement is prepared on one backend and reused on another.
